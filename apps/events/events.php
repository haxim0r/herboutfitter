...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_POST["action"]) && ((strcasecmp($_POST["action"], "create_parent") == 0) || (strcasecmp($_POST["action"], "create_child") == 0 ))){
        ...
        $sql = 'insert into event(type, name, title, description, `start`, `end`, price, `limit`, poster, created_by, `status`, sort) values (\''.
                $_POST['type'].'\', \''.
                $_POST['name'].'\', \''.
                $_POST['title'].'\', \''.
                $_POST['description'].'\', \''.
    ...
    elseif(isset($_POST["action"]) && (strcasecmp($_POST["action"], "remove_node") == 0 )){
        ...
        $sql = 'update event set `status`=\'deleted\' where id='.$_POST["node"];
    }
    elseif(isset($_POST["action"]) && (strcasecmp($_POST["action"], "edit_node") == 0 )){

        $sql = 'update event set type=\''.$_POST['type'].'\', name=\''.
                $_POST['name'].'\', title=\''.
...
function getEventTypes(){
    ...
    $sql = "select id, title from event_type where status='active' order by sort_order, title;";
    
    $mySQL = new db_mysql();
    
    if($daResult = mysqli_query($mySQL->connection, $sql)){
        
        while($row = mysqli_fetch_object($daResult)){
...
<span class='event_list' title="Click to add new event to list."><b>Event console</b></span>
<div id="div_jstree"></div>
<div id="dialog-form" title="Event console" style="display: none;">
    <form enctype="multipart/form-data" method="post" id='newMenuItemForm' name='newMenuItemForm'>
        <fieldset>
            <table>
                ...
                <tr>
                    ...
                    <td><select name="type" id="type"><?=getEventTypes();?></select></td>
                </tr>
                <tr>
...
$sql = "select id, '#' parent, name `text`, '{\"class\":\"event_list\"}' a_attr from event order by sort";
...
?>
    ...
    $.contextMenu({
        ...
        build: function($trigger, e){
            ...
            if(daFirstChild.innerHTML === "Event console"){
                daMenu = {
    ...
    function contextMenuActionSelected(daThing, key, options){
        ...
        if(key === "remove_node"){ //-=-=-=-=-=-=-=- Delete Navigation Node (set record status = 'deleted')
            ...
            if(key === "edit_node"){
                ...
                var sql = 'select * from event where id=' + daNodeId;
                    ...
                    for(var i = 0; i < formData.length; i++){
                        ...
                        for(var key in obj){
                            ...
                            if(document.getElementById(attrName).type.indexOf('select') === 0){
                                ...
                                campaignImage.id = "campaign-image";
