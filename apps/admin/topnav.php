...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_POST['form'])){
        ...
        foreach(explode("&", $_POST['form']) as $kvp){
            ...
            $daPOST[$arrayKVP[0]] = addslashes(urldecode($arrayKVP[1]));
        }
    }

    if(isset($_POST["action"]) && ((strcasecmp($_POST["action"], "create_parent") == 0) || (strcasecmp($_POST["action"], "create_child") == 0 ))){

        $sql = 'insert into site_topnav(text, href, title, sort, parent)values (\''.$daPOST['text'].'\', \''.$daPOST['href'].'\', \''.$daPOST['title'].'\', \''.$daPOST['sort'].'\', \''.$daPOST['parent'].'\')';
    }
    elseif(isset($_POST["action"]) && (strcasecmp($_POST["action"], "remove_node") == 0 )){

        $sql = 'update site_topnav set status=\'deleted\' where id='.$_POST["node"];
    }
    elseif(isset($_POST["action"]) && (strcasecmp($_POST["action"], "edit_node") == 0 )){

        $sql = 'update site_topnav set text=\''.$daPOST['text'].'\', href=\''.$daPOST['href'].'\', title=\''.$daPOST['title'].'\', sort=\''.$daPOST['sort'].'\', parent=\''.$daPOST['parent'].'\', status=\'updated\' where id='.$daPOST['id'];
    }
...
$sql = 'select id, parent, text, \'{"class":"navigation_node"}\' a_attr from site_topnav order by parent, sort';
    ...
    function contextMenuActionSelected(daThing, key, options){
        ...
        if(key === "remove_node"){ //-=-=-=-=-=-=-=- Delete Navigation Node (set record status = 'deleted')
            ...
            if(key === "edit_node"){
                ...
                var sql = 'select * from site_topnav where id=' + daNodeId;
            ...
            function createMenuNode(){
                ...
                request.done(function(msg){
                    ...
                    //console.log(msg);
                    //top.location.reload();
                    navigate('apps/admin/topnav.php');
                });
                request.fail(function(jqXHR, textStatus){
                    top.grayOut(false);
                    alert("Request failed: " + textStatus);
