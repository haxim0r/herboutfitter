...
<div id='div_daHead'>
    ...
    if($_SESSION['auth']['role'] === 'administrator'){
        ...
        $sql = 'select * from site_topnav where parent=\'#\' and status!=\'deleted\' order by sort';
        ...
        if($resultset){
            ...
            while($data = mysqli_fetch_object($resultset)){
                ...
                $sql_inner = 'select * from site_topnav where parent=\''.$data->id.'\' and status!=\'deleted\' order by sort';
        ...
        for(var daCounter = 1; daCounter <= daMenu.length; daCounter++){
            ...
            for(var daCounter_inner = daMenu.length; daCounter_inner > 0; daCounter_inner--){
                ...
                if(daCounter % daCounter_inner == 0){
                    ...
                    daMenuCode += "<center><A CLASS='daMenuLink' HREF=\"" + ((daLink.daURL.length > 0)?daLink.daURL:"#") + "\">" + daLink.daValue + "</A></center>";
